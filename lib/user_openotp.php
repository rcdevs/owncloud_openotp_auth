...
namespace OCA\user_rcdevsopenotp\lib;
...
class OC_USER_OPENOTP extends \OC_User_Backend{
	...
	public function getUsedBackends(){		
		$usedBackends = array();	
		$usedBackends = $this->userManager->getBackends();
		return $usedBackends;
	}
    ...
    public static function registerBackends($usedBackends){
      //\OCP\Util::writeLog('OC_USER_OPENOTP', __FUNCTION__.'().', \OCP\Util::DEBUG);
      if(self::$_backends === null){
        foreach ($usedBackends as $backend){
			if (is_object($backend)) $backend = get_class($backend);
			//\OCP\Util::writeLog('OC_USER_OPENOTP', '----------------------  instance '.$backend.' backend.', 3);
	...
	public function getRealBackend($uid) {
		#\OCP\Util::writeLog('OC_USER_OPENOTP', serialize(debug_backtrace()), 3);
		...
		foreach (self::$_backends as $backend) {
			if ($backend->userExists($uid)) {
				...
				//\OCP\Util::writeLog('OC_USER_OPENOTP', '--------------------------  function getRealBackend() this->_userBackend: ' . get_class($this->_userBackend), \OCP\Util::DEBUG);
				return $this->_userBackend;
			}
		} 
		return null;
    ...
    public function checkPassword($uid, $password) {
		//\OCP\Util::writeLog('OC_USER_OPENOTP', 'function CheckPassword() ' . $uid . " -> " .$password, \OCP\Util::DEBUG);
		$uid = strtolower($uid);
		...
		if(
			...
			if (!$openotpAuth->checkFile('lib/openotp.wsdl','Could not load OpenOTP WSDL file')){
				\OCP\Util::writeLog('OC_USER_OPENOTP', "Could not load OpenOTP WSDL file." , \OCP\Util::ERROR);
			...
			if (!$openotpAuth->checkSOAPext()){
				\OCP\Util::writeLog('OC_USER_OPENOTP', "Your PHP installation is missing the SOAP extension." , \OCP\Util::ERROR);
		 		//form_set_error('openotp', t('An error occured while processing your request, please report error to administrators'));
				return false;
			}
			...
			if( $domain != "" ) \OCP\Util::writeLog('OC_USER_OPENOTP', 'Domain found in username field '.$domain, \OCP\Util::INFO);
			
			if ($state != NULL) {
				// OpenOTP Challenge
				\OCP\Util::writeLog('OC_USER_OPENOTP', 'New OpenOTP Challenge for user ' . $username, \OCP\Util::INFO);
				...
				if( !$userDB->userExists($uid) && $autocreate_user !== "on" ) return false;
					...
					\OCP\Util::writeLog('OC_USER_OPENOTP', 'New OpenOTP SimpleLogin for user ' . $username, \OCP\Util::INFO);
					$resp = $openotpAuth->openOTPSimpleLogin( $username, $domain, utf8_encode($password), $context );
				}
			}
			if (!$resp || !isset($resp['code'])) {
				\OCP\Util::writeLog('OC_USER_OPENOTP', "Invalid OpenOTP response for user " . $username , \OCP\Util::ERROR);
				return false;
			}
			...
			switch ($resp['code']) {
				 case 0:
					...
					\OCP\Util::writeLog('OC_USER_OPENOTP', "OpenOTP Login attempt failed for user " . $username , \OCP\Util::INFO);
					
					if( $authentication_method === AUTHENTICATION_METHOD_STD_OTP ){
						$return = $userDB->checkPassword($uid, $password);
						if($return) \OCP\Util::writeLog('OC_USER_OPENOTP', "User $username has authenticate with Owncloud password" , \OCP\Util::INFO);
						else \OCP\Util::writeLog('OC_USER_OPENOTP', "Standard password Login attempt failed for user " . $username , \OCP\Util::INFO);
						return $return;
					}
					break;
				 case 1:
					\OCP\Util::writeLog('OC_USER_OPENOTP', "User $username has authenticated with OpenOTP." , \OCP\Util::INFO);
					return $username;
					break;
				 case 2:
				 	\OCP\Util::writeLog('OC_USER_OPENOTP', "OpenOTP Response require Challenge" , \OCP\Util::DEBUG);
					...
					break;
				 default:
					\OCP\Util::writeLog('OC_USER_OPENOTP', "Authentication failed for user " . $username , \OCP\Util::ERROR);
					break;
			}			
		} 
	...
	public function userExists( $uid ) {
		//\OCP\Util::writeLog('OC_USER_OPENOTP', 'function userExists() uid:' . $uid, \OCP\Util::DEBUG);
